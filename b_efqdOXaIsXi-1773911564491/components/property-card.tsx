...
import { formatArea, formatPrice, imageSrc, Property } from "@/lib/data";
import type { Locale, SiteSettings } from "@/lib/cms-types";
import { localizeProperty } from "@/lib/i18n-content";
import { localizePath } from "@/lib/i18n";

interface PropertyCardProps {
  property: Property;
  // Localized site settings + locale are passed in by the parent page so we don't
  // re-fetch them once per card (previously an N+1 query on every listing page).
  siteSettings: SiteSettings;
  locale: Locale;
}

export function PropertyCard({ property, siteSettings, locale }: PropertyCardProps) {
    ...
    <Link href={propertyHref} className="group block">
      <article className="bg-white border border-border overflow-hidden transition-all duration-500 hover:shadow-xl hover:shadow-black/5 hover:border-gold/30">
        ...
        <div className="relative aspect-[4/3] overflow-hidden">
          <Image
            src={imageSrc(displayProperty.coverImage ?? displayProperty.images[0])}
